...
*/
  ...
  const reducer = (key, values) => {
    ...
    const docFrequency = uniqueDocs.size;
    ...
    uniqueDocs.forEach(doc => {
      const tf = docData[doc].count / docData[doc].totalWords;
      const tfidf = parseFloat((tf * idf).toFixed(2));
      result[key][doc] = tfidf;
    });
...
test('(10 pts) (scenario) all.mr:strmatch', (done) => {
  ...
  const dataset = [
    ...
    {'doc2': 'This document has no matching pattern'},
    {'doc3': 'Here is another example of matching text'},
    {'doc4': 'No matches in this one either'},
    {'doc5': 'Examples are important for understanding'}
  ];

  const expected = [
    {'matched': ['doc1', 'doc3', 'doc5']}
  ];
